...
class UserInterface:
    def __init__(self, player, presence) -> None:
        ...
        def nothing(): messagebox.showinfo(title="Oops!",
                                           message="This button is still a WIP!")

        # Style definitions
        style = ttk.Style(self.root)
        style.theme_use("classic")
        style.configure("ToolBar.TButton", highlightthickness=-5)
        style.map("ToolBar.TButton", relief=[
                  ("pressed", SUNKEN), ("!active", FLAT), ("active", RAISED)])
        ...
        about_btn = ttk.Button(
            mainframe, text="About", padding="3 2 3 2", style="ToolBar.TButton", command=nothing)
        ...
        player_frame = ttk.Frame(
            mainframe, padding=10, width=466, height=130, border=1, relief=SUNKEN)
        player_frame.grid_propagate(False)
        player_frame.grid(column=1, row=2, columnspan=WCELLS, sticky="ew")

        # Bottom text frame
        listeners_frame = ttk.Frame(
            mainframe, padding=0, width=466, height=20, border=1, relief=SUNKEN)
        listeners_frame.grid_propagate(False)
        listeners_frame.grid(column=1, row=HCELLS,
        ...
        cover_frame = ttk.Frame(player_frame, padding=0,
                                width=110, height=110, border=1, relief=SUNKEN)
        ...
        detandcont_frame = ttk.Frame(
            player_frame, padding="5 0 0 0", width=335, height=110, border=0, relief=SUNKEN)
        ...
        time_frame = ttk.Frame(detandcont_frame, padding=0,
                               width=170, height=25, border=1, relief=SUNKEN)
        ...
        self.time_label = ttk.Label(time_frame, textvariable=self.time_text, font=(
            'Verdana', '10'), justify='center')
        self.time_label.place(x=85, y=11, anchor="center")

        def setvolume(val): player.audio_set_volume(int(round(float(val))))
        volume_scale = ttk.Scale(detandcont_frame, orient=HORIZONTAL,
                                 length=130, from_=0, to=100, command=setvolume, value=100)
        ...
        ctrlbtns_frame = ttk.Frame(
            detandcont_frame, padding="0 0 0 0", height=26, border=0, relief=SUNKEN)
        ...
        self.playbtn_text.set("Play")
        play_btn = ttk.Button(
            ctrlbtns_frame, textvariable=self.playbtn_text, padding="3 2 3 2", command=play)
        play_btn.grid(column=1, row=1, sticky="we")

        self.like_btn_text = StringVar()
        self.like_btn_text.set("♥ 0")
        like_btn = ttk.Button(ctrlbtns_frame, textvariable=self.like_btn_text,
                              padding="3 2 3 2", width=4, command=nothing)
        like_btn.grid(column=2, row=1, sticky="we", padx=6)

        settings_btn = ttk.Button(
            ctrlbtns_frame, text="Settings", padding="3 2 3 2", command=nothing)
    ...
    def set_time(self, length, position, end):
        ...
        self.time_text.set(
            "{}:{} / {}:{}".format(timepos[1], timepos[2], timelen[1], timelen[2]))
    ...
    def set_listeners(self, count):
        self.listeners_text.set("Listeners: {}".format(count))
